  ...
  contract.on('ProposalCreated', async (proposalId: any, contentHash: any, proposer: any, event: any) => {
    try {
      ...
      const result: any = await resonate.rpc(
        ...
        'scoreProposal',
        Number(proposalId),
        resonate.options({ target: 'poll://any@proposal-scorers' })  // Routes to workers!
      );
 ...
 */
async function getMerkleRoot(resonate: CryptoResonate, promiseId: string): Promise<string> {
  try {
