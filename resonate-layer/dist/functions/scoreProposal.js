...
exports.scoreProposal = scoreProposal;
const fetchVotes_1 = require("./fetchVotes");
const getReputations_1 = require("./getReputations");
const checkEligibility_1 = require("./checkEligibility");
 ...
 */
function* scoreProposal(ctx, proposalId) {
    ...
    const votes = yield* ctx.run(fetchVotes_1.fetchVotes, proposalId, ctx.options({ id: 'fetchVotes' }));
    // Step 2: Calculate voter reputations (durable child promise)
    console.log('\n🎯 Step 2/4: Calculating reputations...');
    const reputations = yield* ctx.run(getReputations_1.getReputations, votes, ctx.options({ id: 'getReputations' }));
    // Step 3: Check voter eligibility (durable child promise)
    console.log('\n✅ Step 3/4: Checking eligibility...');
    const eligibility = yield* ctx.run(checkEligibility_1.checkEligibility, votes, proposalCreatedAt, ctx.options({ id: 'checkEligibility' }));
    // Step 4: Calculate final score (durable child promise)
    console.log('\n🧮 Step 4/4: Calculating final score...');
    const result = yield* ctx.run(calculateScore_1.calculateScore, votes, reputations, eligibility, proposalCreatedAt, ctx.options({ id: 'calculateScore' }));
    console.log('\n' + '─'.repeat(60));
